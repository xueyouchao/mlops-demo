...
- **Existing surface to build on** (verified by reading the code, not assumed):
  - Activities: `promote_stage`, `train_and_register` in `services/worker/activities.py`.
  - Workflows: `TrainingWorkflow`, `PromotionWorkflow` in `services/worker/workflows.py`, registered in `services/worker/main.py`. (A `RollbackWorkflow`/`rollback_stage` pair existed but nothing ever started it — rollback is the api route `POST /api/models/rollback`, synchronously — so it was deleted.)
